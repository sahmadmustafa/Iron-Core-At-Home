...
- (void)prepareMenuItemView:(UIView *)menuItemView date:(NSDate *)date
{
    ...
    if(date){
        ...
        textYear = [df stringFromDate:date];
        
        
//        textYear = [[dateFormatter standaloneYea][currentYearIndex - 1] capitalizedString];

    }
        
    [(UILabel *)menuItemView setText:[NSString stringWithFormat:@"%@ %@",text,textYear]];
}
